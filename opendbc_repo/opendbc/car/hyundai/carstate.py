...
class CarState(CarStateBase):
  def __init__(self, CP):
    ...
    self.main_enabled = True if Params().get("AutoEngage") == 2 else False
  ...
  def update(self, can_parsers) -> structs.CarState:
    ...
    if self.CP.openpilotLongitudinalControl:
      ...
      ret.radarDRel = cp_cruise.vl["SCC11"]["ACC_ObjDist"]
      ret.radarVRel = cp_cruise.vl["SCC11"]["ACC_ObjRelSpd"]
      ret.vSetDis = cp_cruise.vl["SCC11"]["VSetDis"]
  ...
  def update_canfd(self, can_parsers) -> structs.CarState:
    ...
    ret.standstill = cp.vl["WHEEL_SPEEDS"]["WHEEL_SPEED_1"] <= STANDSTILL_THRESHOLD and cp.vl["WHEEL_SPEEDS"]["WHEEL_SPEED_2"] <= STANDSTILL_THRESHOLD and \
                     cp.vl["WHEEL_SPEEDS"]["WHEEL_SPEED_3"] <= STANDSTILL_THRESHOLD and cp.vl["WHEEL_SPEEDS"]["WHEEL_SPEED_4"] <= STANDSTILL_THRESHOLD

    ret.brakeLights = ret.brakePressed or cp.vl["TCS"]["BrakeLight"] == 1 or ret.brakeHoldActive or self.i_pedel_stop
    ...
    if self.CP.flags & HyundaiFlags.CAMERA_SCC.value:
      ...
      ret.aReqValue = cp_cam.vl["SCC_CONTROL"]["aReqValue"]
      ret.radarDRel = cp_cam.vl["SCC_CONTROL"]["ACC_ObjDist"]
      ret.radarVRel = cp_cam.vl["SCC_CONTROL"]["ACC_ObjRelSpd"]
      ret.vSetDis = cp_cam.vl["SCC_CONTROL"]["VSetDis"]
    ...
    if self.CP.flags & HyundaiFlags.CAMERA_SCC.value:
      ...
      self.clu_spd_lim_info = cp.vl["CLUSTER_SPEED_LIMIT"] if self.CLU_SPD_LIM_INFO else None
      if self.clu_spd_lim_info is not None:
        if self.clu_spd_lim_info["SPEED_LIMIT_1"] != 0:
          ret.roadLimitSpeed = self.clu_spd_lim_info["SPEED_LIMIT_1"]
    ...
    vEgoClu, aEgoClu = self.update_speed_kf(ret.vEgoCluster)
    ret.vCluRatio = (ret.vEgo / vEgoClu) if (vEgoClu > 3. and ret.vEgo > 3.) else 1.0
