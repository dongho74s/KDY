...
class CarrotServ:
  ...
  def update_params(self):
    self.autoNaviSpeedBumpSpeed = float(self.params.get("AutoNaviSpeedBumpSpeed"))
    self.autoNaviSpeedBumpTime = float(self.params.get("AutoNaviSpeedBumpTime"))
    ...
    self.autoTurnControlSpeedTurn = self.params.get("AutoTurnControlSpeedTurn")
  ...
  def _update_sdi(self):
    ...
    if self.nSdiType in [0,1,2,3,4,7,8, 75, 76] and self.nSdiSpeedLimit > 0 and self.autoNaviSpeedCtrlMode > 0:
      self.xSpdLimit_org = self.nSdiSpeedLimit
  ...
  def update_navi(self, remote_ip, sm, pm, vturn_speed, coords, distances, route_speed, gps_service):
    ...
    if sm.alive['carState'] and sm.alive['selfdriveState']:
      ...
      if CS.roadLimitSpeed > 0 and self.active_carrot <= 1:
        self.nRoadLimitSpeed = CS.roadLimitSpeed
    ...
    msg.carrotMan.xSpdLimitOrg = int(self.xSpdLimit_org)
